...
### Запуск из образов Docker Hub (без сборки)

Если образы выложены в Docker Hub, другой пользователь может запустить проект без сборки: см. **[docs/DOCKER_HUB.md](docs/DOCKER_HUB.md)** — там пошагово описано, как выложить образы (разработчик) и как запустить у себя (руководитель).
